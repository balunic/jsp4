...
<body>
	<form class="form-signin">
		...
		<label for="pwd" class="sr-only">Password</label> <input
			...
			placeholder="Password" required> <label for="id"
			class="sr-only">이름</label> <input type="text" id="name" name="name"
			class="form-control" placeholder="이름" required> <label
			for="id" class="sr-only">나이</label> <input type="text" id="age"
			name="age" class="form-control" placeholder="나이" required> <label
			for="id" class="sr-only">주소</label> <input type="text" id="address"
	...
	function callback(result){
		...
		for(var di of result){
			str+= "<option value='" + di.doNo+"'>"+di.diName+"</option>"
		}
		$("#dino").html(str);
	}
